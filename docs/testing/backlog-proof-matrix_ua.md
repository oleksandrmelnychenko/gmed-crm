...
> Канонічна матриця `backlog -> proof`, актуалізована **2026-08-23**. Цей файл читається разом з [`docs/backlog/01_mvp-backlog_ua.md`](../backlog/01_mvp-backlog_ua.md) і [`p0-operations-release-proof_ua.md`](p0-operations-release-proof_ua.md) та показує не “що колись планувалось”, а який тип доказу вже є для кожного MVP epic:
>
...
| `R5 Interpreter and Concierge Operations` | interpreter assignment, briefing, travel/concierge ops, task/event manager, internal/external assignees, receipts | `appointment_care_path_api.rs`, `concierge_operational_items_api.rs`, `concierge_expense_receipts_api.rs`, `operations_p0_api.rs` | `appointments-staff.live.spec.ts`, `patient-portal.live.spec.ts`, `operations-p0.live.spec.ts` | `Confirmed` | none inside internal scope |
| `R6 Document Flow` | upload, classify, share, provider-only medical share, provider/patient document links, internal notes attachments | `documents_api.rs`, `provider_templates_api.rs`, `operations_p0_api.rs` | `staff-workflows.live.spec.ts`, `patient-portal.live.spec.ts`, `operations-p0.live.spec.ts` | `Confirmed` | none inside internal scope |
| `R7 Billing and Cost Control` | billing aggregation, quotes/invoices/dunning, concierge receipt posting, patient/company balances, accounting export | `invoices_api.rs`, `concierge_expense_receipts_api.rs`, `patient_account_statement_api.rs`, `company_financial_*_api.rs`, `operations_p0_api.rs` | `commercial.live.spec.ts`, `appointments-staff.live.spec.ts`, `operations-p0.live.spec.ts` | `Partial` | `DATEV`, `E-Rechnung`, real payment settlement/provider checkout remain open |
...
| `R9 Communication` | internal case comms, internal notes with files, clinic/provider comms, tasks, secure patient messaging | `messages_api.rs`, `messages_portal_api.rs`, `operations_p0_api.rs`, `workflow_checklists_api.rs` | `chat-secure.live.spec.ts`, `staff-workflows.live.spec.ts`, `operations-p0.live.spec.ts` | `Confirmed` | none inside internal scope |
