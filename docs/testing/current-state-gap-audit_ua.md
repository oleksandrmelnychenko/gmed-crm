...
- Browser-level regression harness already covers more than shell navigation: staff UI path for document portal release/revoke, provider share/revoke with cover message and template generation, recurring appointment whole-series cancellation from the detail drawer, secure chat text-send with local keyring/mock envelope path, plus patient portal invoice payment-proof upload and self-service export/privacy-request submission.
