...
## Вихідні e-рахунки: перевірка офіційним валідатором — 2026-09-19

Вихідний ZUGFeRD/Factur-X (`crates/server/src/routes/invoices/zugferd.rs`,
коміт `b6e12b62`) прогнано через офіційний валідатор Mustang 2.26.0
(Schematron EN 16931 / Factur-X). Знайдено й виправлено два порушення:

- **BR-CO-26**: продавець без USt-IdNr. не мав ідентифікатора. Тепер
  Steuernummer додатково пишеться як `SellerTradeParty/ram:ID` (BT-29), окрім
  `SpecifiedTaxRegistration schemeID="FC"` (BT-32).
- **BR-E-01/BR-E-08**: медичні послуги та durchlaufende Posten утворювали дві
  групи `E`; стандарт дозволяє рівно одну. Тепер одна група зі списком підстав.
  Підстава для ставки 0 % — «Steuerfreie Heilbehandlung nach § 4 Nr. 14 UStG»
  (код `VATEX-EU-132`, лише коли всі звільнені позиції медичні).

Новий CI job `einvoice-validation` експортує зразки з unit-тестів
(`EINVOICE_SAMPLE_DIR`) і вимагає `status="valid"` від Mustang; jar
