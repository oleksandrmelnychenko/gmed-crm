...
export function OrderWizard({ patient, orderId, onClose, onCreated, onSaved }: {
  ...
  const specializationById = new Map([...(data?.catalog_snapshot?.specializations ?? []), ...workCatalog.specializations].map(item => [item.id, item]));
  ...
  return <OrderWizardShell dirty={dirty} busy={busy} loading={loading} disabled={busy || loading || !data}
    ...
    {loading ? <p role="status" className="flex gap-2 py-8"><LoaderCircle className="size-4 animate-spin" />{tx("Загрузка…", "Wird geladen…")}</p> : !data ?
      ...
      <fieldset disabled={busy} className="min-w-0 space-y-3 disabled:opacity-75">
        ...
        {step === 2 ? <>
          <OrderWizardLinesTable lines={data.lines} catalog={orderCatalog} effectiveOn={data.date_from} dateTo={data.date_to} specialties={selectedSpecializations.map(item => item.name_de || item.name_en || item.code)} lang={lang} busy={busy} onChange={lines => patch({ lines })}>
            {data.needs_description ? <div className="rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-xs text-amber-950">{data.needs_description}</div> : null}
            {selectedWorkTypes.length ? <div className="space-y-2"><h4 className="flex items-center gap-2 text-xs font-semibold"><span className="size-2 rounded-full bg-orange-500" />{tx("Виды работ и часы", "Leistungsarten und Stunden")}</h4><SelectedWorkTypesSummary workTypes={selectedWorkTypes} specializationLabels={new Map(selectedSpecializations.map(item => [item.id, intakeSpecializationName(item, lang)]))} lang={lang} tx={tx} /></div> : null}
        ...
        {step === 5 ? <OrderWizardSection title={tx("Проверка перед оформлением", "Prüfung vor Abschluss")}>
          <dl className="grid gap-3 rounded-lg border border-border/60 bg-muted/20 p-3 sm:grid-cols-3 [&_dd]:mt-1 [&_dd]:font-mono [&_dd]:text-sm [&_dd]:font-medium"><div><dt className="text-xs text-muted-foreground">{tx("Период", "Zeitraum")}</dt><dd>{periodLabel}</dd></div><div><dt className="text-xs text-muted-foreground">{tx("Стоимость", "Gesamtpreis")}</dt><dd>{amountLabel}</dd></div><div><dt className="text-xs text-muted-foreground">{tx("Договор", "Vertrag")}</dt><dd>{selectedContract?.contract_number ?? "—"}</dd></div></dl>
