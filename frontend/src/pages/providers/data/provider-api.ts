...
function normalizeProviderDetail(raw: ProviderDetail): ProviderDetail {
  return {
    ...
    taxonomy_node_ids: arrayOrEmpty<string>(raw.taxonomy_node_ids),
    taxonomy_filter_ids: arrayOrEmpty<string>(raw.taxonomy_filter_ids),
...
export function fetchProviders(path: string) {
  return apiFetch<unknown>(path).then((rows) => {
    ...
    return (rows as ProviderSummary[]).map((provider) => ({
      ...
      taxonomy_node_ids: arrayOrEmpty<string>(provider.taxonomy_node_ids),
      taxonomy_filter_ids: arrayOrEmpty<string>(provider.taxonomy_filter_ids),
