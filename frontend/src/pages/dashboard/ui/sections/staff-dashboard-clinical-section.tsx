...
export function StaffDashboardClinicalSection({
  ...
  return (
    <>
      <SectionHeader
        title={tr.dash_sec_clinical ?? "Clinical"}
        hint={tr.dash_sec_clinical_hint ?? "Case reasons, statuses and treatment patterns"}
      />
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_220px]">
        ...
        <ChartCard
          title={tr.dash_cases_by_status ?? "Cases by status"}
          hint={tr.dash_cases_status_hint ?? "Pipeline snapshot"}
        >
          <MiniDonut
            loading={sectionsLoading}
        ...
        <div className="relative flex flex-col justify-between overflow-hidden rounded-xl border border-border bg-card p-4">
          ...
          <div className="relative">
            <span className="text-[12px] text-muted-foreground">
              {tr.dash_avg_duration ?? "Avg case duration"}
            </span>
      ...
      <ChartCard
        title={tr.dash_service_mix ?? "Service mix"}
        hint={tr.dash_service_mix_hint ?? "Procedures by volume and revenue"}
      >
