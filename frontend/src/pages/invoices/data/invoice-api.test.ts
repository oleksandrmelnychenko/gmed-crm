...
describe("invoice billing preflight", () => {
  it("creates the invoice without a separate service approval step", async () => {
    request
      .mockResolvedValueOnce({ process_gates: { billing_release_status: "granted" }, leistungen: [{ id: "service-1", status: "planned" }] })
      .mockResolvedValueOnce({ id: "invoice" });
    await expect(createInvoice("quote-1", payload, "order-1")).resolves.toEqual({ id: "invoice" });
  });
