...
export function StructuredInvoiceDetails({ preview, labels }: { preview: InvoiceImportPreview; labels: Record<keyof InvoiceImportFields, string> }) {
  ...
  return <>
    ...
    {warnings.includes("mixed_tax_treatment_requires_review") ? <Banner tone="warning" withIcon>{tx("Счёт содержит части с разным режимом НДС. Итоги сверены арифметически, но налоговую категорию каждой части нужно подтвердить перед бухгалтерским учётом.", "Die Rechnung enthält Positionen mit unterschiedlicher Umsatzsteuerbehandlung. Die Summen wurden rechnerisch abgeglichen; die Steuerkategorie jeder Position muss vor der Buchung geprüft werden.")}</Banner> : null}
    {preview.tax_breakdown?.length ? <div className="space-y-1 text-xs"><p className="font-medium">{preview.structured ? tx("НДС по XML", "Umsatzsteuer laut XML") : tx("Разбивка НДС из документа", "Umsatzsteuer-Aufteilung laut Dokument")}</p>{preview.tax_breakdown.map((tax, index) => <p key={index}>{tax.label ?? tax.category ?? "—"} · {tax.rate != null ? `${tax.rate} %` : tx("ставка не указана", "Steuersatz nicht angegeben")} · {tx("База", "Basis")}: {tax.base ?? "—"} · {tx("НДС", "USt.")}: {tax.amount ?? "—"} {preview.fields.currency}</p>)}</div> : null}
  </>;
}
