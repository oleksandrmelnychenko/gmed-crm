-- Preliminary medical estimates have their own compact numbering sequence.
CREATE SEQUENCE IF NOT EXISTS cost_estimate_document_number_seq START 1;

WITH existing_numbers AS (
    SELECT MAX(
               substring(
                   document_number
                   FROM '^VKS-[0-9]{8}-([0-9]{1,9})$'
               )::BIGINT
           ) AS maximum
      FROM documents
)
SELECT setval(
           'cost_estimate_document_number_seq',
           COALESCE(maximum, 1),
           maximum IS NOT NULL
       )
...
BEGIN
    ...
    IF NEW.generated_template_id = 'cost_estimate' THEN
        sequence_value := nextval('cost_estimate_document_number_seq');
        NEW.document_number := format(
            'VKS-%s-%s',
