-- A preliminary medical cost estimate is an independent document. It must not
-- inherit either the single-order number or the quote number.
...
BEGIN
    ...
    ELSIF NEW.generated_template_id = 'order_cost_estimate' THEN
    ...
    IF NEW.generated_template_id = 'cost_estimate' THEN
        NEW.document_number := format(
            'VKS-%s-%s',
            to_char(COALESCE(NEW.document_date, CURRENT_DATE), 'YYYYMMDD'),
            upper(left(replace(NEW.id::text, '-', ''), 12))
        );
        RETURN NEW;
    END IF;
...
SET document_number = format(
        'VKS-%s-%s',
        to_char(COALESCE(document_date, created_at::date), 'YYYYMMDD'),
        upper(left(replace(id::text, '-', ''), 12))
    ),
    updated_at = now()
WHERE generated_template_id = 'cost_estimate'
  AND document_number !~ '^VKS-[0-9]{8}-[A-F0-9]{12}$';
