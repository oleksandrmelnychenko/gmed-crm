...
pub(crate) async fn resolve_linked_patient_id(
    ...
    if let Some(row) = rows.first() {
        let patient_id = row.try_get::<Uuid, _>("patient_id").map_err(|e| {
            tracing::error!(error = %e, user_id = %user_id, "decode patient_id from assignment row");
            err(
                StatusCode::INTERNAL_SERVER_ERROR,
                "Failed to validate linked patient",
            )
    ...
    let patient_id = row.try_get::<Uuid, _>("patient_id").map_err(|e| {
        tracing::error!(error = %e, user_id = %user_id, "decode patient_id from email-linked row");
        err(
            StatusCode::INTERNAL_SERVER_ERROR,
            "Failed to validate linked patient",
        )
