...
fn recommendation_select_sql(where_clause: &str) -> String {
    format!(
        r#"SELECT pr.id, pr.patient_id, pr.title, pr.description, pr.recommendation_type,
                  ...
                  doctor.title AS source_doctor_title,
                  doctor.fachbereich AS source_doctor_fachbereich,
...
fn recommendation_json(row: &sqlx::postgres::PgRow) -> serde_json::Value {
    json!({
        ...
        "source_doctor_title": row.try_get::<Option<String>, _>("source_doctor_title").unwrap_or_default(),
        "source_doctor_fachbereich": row.try_get::<Option<String>, _>("source_doctor_fachbereich").unwrap_or_default(),
