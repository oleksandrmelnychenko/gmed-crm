...
#[tokio::test]
async fn lead_edit_lease_allows_one_editor_and_releases_cleanly() {
    let Some(app) = test_app().await else { return };
    let owner_bearer = app.auth_header("patient_manager");
    let second_user_id = seed_user(&app.suite.pool, "lead-lease-second", "patient_manager").await;
    let second_bearer = format!(
        "Bearer {}",
    ...
    let (status, created) = json_request(
        ...
        "/api/v1/leads",
        &owner_bearer,
        Some(json!({ "first_name": "Single", "last_name": "Editor" })),
    )
    .await;
    assert_eq!(status, StatusCode::CREATED, "{created}");
    let lead_id = created["id"].as_str().unwrap();

    let lease_path = format!("/api/v1/leads/{lead_id}/edit-lease");
    let (status, owner_lease) = json_request(&app, "POST", &lease_path, &owner_bearer, None).await;
    assert_eq!(status, StatusCode::OK, "{owner_lease}");
    assert_eq!(owner_lease["editable"], true);
    assert_eq!(
        owner_lease["holder"]["user_id"],
        app.patient_manager_id.to_string()
    );

    let (status, blocked_lease) =
        json_request(&app, "POST", &lease_path, &second_bearer, None).await;
    assert_eq!(status, StatusCode::CONFLICT, "{blocked_lease}");
    assert_eq!(blocked_lease["error"], "lead_edit_locked");
    assert_eq!(
        blocked_lease["holder"]["user_id"],
        app.patient_manager_id.to_string()
    );

    let (status, blocked_update) = json_request(
        &app,
        "POST",
        &format!("/api/v1/leads/{lead_id}/update"),
        &second_bearer,
    ...
    .await;
    assert_eq!(status, StatusCode::CONFLICT, "{blocked_update}");
    assert_eq!(blocked_update["error"], "lead_edit_locked");

    let (status, owner_update) = json_request(
        &app,
        "POST",
        &format!("/api/v1/leads/{lead_id}/update"),
        &owner_bearer,
        Some(json!({ "first_name": "Saved" })),
    )
    .await;
    ...
    let (status, released) = json_request(
        &app,
        "POST",
        &format!("/api/v1/leads/{lead_id}/edit-lease/release"),
        &owner_bearer,
        None,
    )
    .await;
    ...
    let (status, second_lease) =
        json_request(&app, "POST", &lease_path, &second_bearer, None).await;
    assert_eq!(status, StatusCode::OK, "{second_lease}");
    assert_eq!(
        second_lease["holder"]["user_id"],
